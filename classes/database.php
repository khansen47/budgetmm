...
class Database{
	...
	public function __construct(){
		$this->mysqli = new mysqli("localhost", "khansen", "123", "money_time");
...
class Database2 extends mysqli
{
	...
	public function __construct()
	{
		parent::__construct( 'localhost', 'khansen',  '123', 'money_time' );
	...
	public function single2( $query, &$results )
	{
		$results 	= array();
		$args		= func_get_args();
	...
	private function RunStatement( &$query, $arg_list, $multiple_results = false, &$results = null, &$count = 0 )
	{
		...
		{
			return Functions::Error( 'BMM-DATABASE-0', $this->error );
		}
		...
		if ( $arg_count > $min_arg_number )
		{
			...
			{
				return Functions::Error( 'BMM-DATABASE-1', $this->error );
			}
		}

		if ( !$stmt->execute() )
		{
			return Functions::Error( 'BMM-DATABASE-2', $this->error );
		}
		...
		if ( !is_null( $results ) )
		{
			...
			{
				return Functions::Error( 'BMM-DATABASE-3', $this->error );
			}
			...
			if ( !$count = $stmt->num_rows )
			{
				if ( !$stmt->close() )
				{
					return Functions::Error( 'BMM-DATABASE-4', $this->error );
				}
			...
			{
				return Functions::Error( 'BMM-DATABASE-5', $this->error );
			}
			...
			{
				return Functions::Error( 'BMM-DATABASE-6', $this->error );
			}
			...
			if ( $multiple_results )
			{
				...
				if ( !$stmt->fetch() )
				{
					return Functions::Error( 'BMM-DATABASE-7', $this->error );
				}
		...
		if ( !$stmt->close() )
		{
			return Functions::Error( 'BMM-DATABASE-8', $this->error );
		}
